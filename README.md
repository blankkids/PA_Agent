...
面向主观交易者的 **价格行为（Price Action）** AI 辅助决策工具。从 **MT5 / TradingView / AkShare / 东方财富 / TuShare / yfinance** 读取 K 线，将结构化 K 线数据与预计算特征送入大模型做**两阶段分析**（市场诊断 → 交易决策），**不是**截图识图，**不连接券商、不执行下单**。

---

## 主要功能

- 📈 **多数据源**：MT5（Windows）、TradingView（全平台）、AkShare / 东方财富 / TuShare（A 股）、yfinance（期货/加密货币）
...
| Python   | 3.11+                                                                    |
| 数据源   | MT5 / TradingView / AkShare / 东方财富 / TuShare **至少配置一种**         |
